...
export class CourseService {
  ...
  async getCourseList(filters: CourseListFilters) {
    return await courseRepository.getList(filters);
  }

  async getAvailableCourses() {
    return await courseRepository.getAvailableCourses();
  }
  ...
  async getCourseByOrganizationAndSlug(
    organizationId: string,
    slug: string,
  ) {
    return await courseRepository.getByOrganizationAndSlug(
      organizationId,
      slug,
    );
  }

  async getCoursesByOrganization(organizationId: string) {
    return await courseRepository.getByOrganization(organizationId);
  }
  ...
  private async validateProgramOwnership(
    organizationId: string,
    programId: string,
  ) {
    ...
    if (!program || program.organization_id !== organizationId) {
      throw new Error(
        "Program tidak sesuai dengan Organization yang dipilih.",
      );
    }
  }

  private async validateSlugAvailability(
    organizationId: string,
    slug: string,
    excludeId?: string,
  ) {
    ...
    if (existing) {
      throw new Error(
        "Slug course sudah digunakan pada Organization yang sama.",
      );
    }
  }
}
