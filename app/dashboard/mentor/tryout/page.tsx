...
interface TryoutMentorPageProps {
...
export default async function TryoutMentorPage({
  searchParams,
}: TryoutMentorPageProps) {
  ...
  const tryouts = await tryoutService.getMentorTryouts(profile.id);
  ...
  return (
    <main className="mx-auto w-full max-w-7xl space-y-7 p-4 sm:p-6 lg:p-8">
      <PageHeader
        title="Try Out"
        description="Buat dan publikasikan Try Out hanya untuk course yang ditugaskan kepada Anda."
        actions={
          <Link
            href="/dashboard/mentor/tryout/new"
      ...
      {filtered.length === 0 ? (
        <section className="rounded-3xl border border-dashed border-blue-200 bg-white p-10 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Tambahkan Try Out pada course yang ditugaskan kepada Anda.
          </p>
        </section>
      ) : (
          ...
          {filtered.map((tryout) => (
            ...
            >
              ...
              <div className="mt-5 grid gap-2 sm:grid-cols-3">
                <Link
                  href={`/dashboard/mentor/tryout/${tryout.id}/edit`}
                ...
                <Link
                  href={`/dashboard/mentor/tryout/${tryout.id}/questions`}
                ...
                <Link
                  href={`/dashboard/mentor/tryout/${tryout.id}/results`}
