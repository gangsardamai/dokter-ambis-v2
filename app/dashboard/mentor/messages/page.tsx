...
export default async function MentorMessagesPage({
  ...
  const threads = await lessonMessageService.getMentorInbox(profile.id);
  ...
  return (
    <main className="mx-auto w-full max-w-7xl space-y-7 p-4 sm:p-6 lg:p-8">
      <PageHeader
        title="Kotak Pesan"
        description="Jawab pertanyaan peserta hanya dari course yang ditugaskan kepada Anda."
      />
      ...
      <section className="space-y-3">
        {filtered.length === 0 ? (
          ...
          filtered.map((thread) => (
            <Link
              key={thread.id}
              href={`/dashboard/mentor/messages/${thread.id}`}
